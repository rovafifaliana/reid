...
class Checkpointer(object):
    ...
    def _load_model(self, checkpoint):
        ...
        # Load with strict=False to ignore missing/mismatched keys
        load_result = self.model.load_state_dict(checkpoint_state_dict, strict=False)
        
        # Convert to expected _IncompatibleKeys format
        return _IncompatibleKeys(
            missing_keys=load_result.missing_keys,
            unexpected_keys=load_result.unexpected_keys,
            incorrect_shapes=[]  # Empty list as we're not tracking shape mismatches separately
        )
    
    def _check_model_state_dict(self, checkpoint_dict, model_dict):
        """
        Check for shape mismatches between checkpoint and model.
        Returns a dictionary of incompatible keys.
        """
        incompatible = {
            'missing_keys': [],
            'unexpected_keys': [],
        ...
        for k in model_dict:
            if k not in checkpoint_dict:
                incompatible['missing_keys'].append(k)
            elif checkpoint_dict[k].shape != model_dict[k].shape:
                incompatible['shape_mismatch'].append(k)
                
        for k in checkpoint_dict:
            if k not in model_dict and not k.startswith(('optimizer', 'lr_sched', 'warmup_sched', 'epoch', 'metric')):
                incompatible['unexpected_keys'].append(k)
    ...
    def _log_incompatible_keys(self, incompatible: _IncompatibleKeys) -> None:
        ...
        # Skip shape checking if attribute doesn't exist
        if hasattr(incompatible, 'incorrect_shapes'):
    ...
    def _convert_ndarray_to_tensor(self, state_dict):
        """
        Convert all numpy arrays in the state_dict to torch tensors.
        Handles:
        - OrderedDict
        - Regular tensors/arrays
        - Skips optimizer and other non-model parameters
        """
        for k in list(state_dict.keys()):
            v = state_dict[k]
            
            # Skip optimizer and other training-specific parameters
            if k in ['optimizer', 'lr_sched', 'warmup_sched', 'epoch', 'metric']:
                del state_dict[k]
                continue
                
            # Handle OrderedDict case
            if isinstance(v, collections.OrderedDict):
                self._convert_ndarray_to_tensor(v)
                continue
                
            # Handle regular dictionaries (like optimizer state)
