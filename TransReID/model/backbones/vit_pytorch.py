...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Vérification de la forme du tenseur
        if qkv.shape[0] != 3:
            raise ValueError(f"Expected qkv to have first dimension 3, got {qkv.shape[0]}")
        
        q, k, v = qkv[0], qkv[1], qkv[2]  # q, k, v each have shape (B, num_heads, N, C // num_heads)
    # def forward(self, x):
    #     B, N, C = x.shape
    #     qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class TransReID(nn.Module):
    ...
    # In your vit_pytorch.py or where load_param is defined
    def load_param(self, model_path):
        checkpoint = torch.load(model_path)
        
        # Handle different checkpoint formats
        if 'model' in checkpoint:
            param_dict = checkpoint['model']  # Most common case
        elif 'state_dict' in checkpoint:
            param_dict = checkpoint['state_dict']
        else:
            param_dict = checkpoint  # Assume direct weights
        ...
        for k, v in param_dict.items():
            if k in model_dict and v.shape == model_dict[k].shape:
                ...
                print(f"Skipping parameter {k} (shape mismatch or not in model)")
        
        # Handle special cases for TransReID architecture
        if 'pos_embed' in new_dict and hasattr(self, 'pos_embed'):
            # Resize positional embedding if needed
            old_pos_embed = new_dict['pos_embed']
            new_pos_embed = self.pos_embed
            if old_pos_embed.shape != new_pos_embed.shape:
                print("Adjusting positional embedding size...")
